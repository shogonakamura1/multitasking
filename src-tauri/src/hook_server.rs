...
fn extract_tasks_via_llm(
    ...
    let system = "あなたは非常に厳格なタスク抽出器です。出力は必ず日本語のみ（中国語・英語・前置き・説明文・記号は禁止）。\
本当に実行が必要だと確信できる作業だけを出力し、少しでも迷うものは出力しない。該当が無ければ完全に空（空行も無し）で返す。タスク名以外は一切出力しない。";

    let prompt = format!(
        "プロジェクト「{project_name}」での、ユーザーの依頼とAIの応答です。\n\
ここから、ユーザーが次に実行する必要のある『確定した具体的な作業』だけを、後から単独で見ても分かるタスク名として抽出してください。\n\n\
【含める条件（すべて満たすものだけ）】\n\
・ユーザー自身が「やる」と決めた／依頼した具体的な作業（AIが提案しただけのものは不可）\n\
・まだ完了していない（応答に「実装した」「完了」「済」とある作業は除外）\n\
・対象が具体的で、『対象＋動作』の形で一文にできる\n\n\
【含めないもの】\n\
・AIが提案・推奨しただけで、ユーザーが採用を明言していない項目（例「〜しましょうか？」「推奨は」「イチオシ」「〜してはどうか」「①で進めますか？」への未返答）\n\
・単なる提案・選択肢・未確定アイデアの列挙\n\
・一般的な説明・背景・考察・感想\n\
・質問・確認・相談（例「どれにする？」）\n\
・すでに完了した作業\n\
・対象が曖昧なもの、抽象的なもの\n\n\
【厳守】\n\
・迷ったら含めない。確信できる必須タスクだけ。多くても2件。\n\
・該当が無ければ何も出力しない（空で返す）。\n\
・1行1タスク。箇条書き記号や番号は付けない。\n\n\
良い例（確定した具体作業）:\n\
汀線残差に対する白波インデックス(surf index)の相関分析を実装する\n\
二見ヶ浦の全596シーンで白波量と汀線残差の散布図を作成する\n\n\
出力してはいけない例:\n\
・「道A/道B/道C のどれにするか相談」→ 未確定なので何も出さない\n\
・「Otsu法に変えると安定します（実装済み）」→ 完了済みなので出さない\n\
・「実装する」「精度を上げる」→ 対象が無く曖昧なので出さない\n\n\
[依頼]\n{user_text}\n\n[応答]\n{assistant_text}"
    );
